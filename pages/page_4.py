...
with st.form(key="Not_working"):
    ...
    if st.session_state.data.get("availability") == None:
        st.session_state.data["availibality"] = st.radio("Are you available to work in the next 2 weeks?", YN, index=None)
    else:
        st.session_state.data["availibality"] = st.radio("Are you available to work in the next 2 weeks?", YN, index=YN.index(st.session_state.data.get("availability")))
